...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int k = 0; k < groupn; k++) {
            ...
            for (int i = k; i < n - 1 - k; i++) { //1 ~ n
                group.add(graph[i][m - k - 1]);
            }
        ...
        for (int k = 0; k < groupn; k++) {
            ...
            for (int i = k; i < n - 1 - k; i++, index = (index+1)%len) { //1 ~ n
               graph[i][m - k - 1] = group.get(index);
            }

            for (int j = m - 1 - k; j > k; j--, index = (index+1)%len) {
                graph[n - 1 - k][j] = group.get(index);
            }
            for (int i = n - 1 - k; i > k; i--, index = (index+1)%len) {
