    ...
    public static void movePawn(int T) {
        for (int k = 0; k < K; k++) {
            ...
            for (Pawn p : pawnGraph[x][y]) {
                if(p.idx == pawnList[k].idx) break;
                targetIdx++;
            }
            int dir = pawnGraph[x][y].get(targetIdx).dir;
            int nX = x + dx[dir];
            int nY = y + dy[dir];

            if(0<=nX && nX < N && 0<=nY && nY < N){
                if(board[nX][nY] == BLUE) {
                    pawnGraph[x][y].get(targetIdx).dir = opponent(dir);
                }
            }else{
                pawnGraph[x][y].get(targetIdx).dir = opponent(dir);
            }

            dir = pawnGraph[x][y].get(targetIdx).dir;
            nX = x + dx[dir];
            nY = y + dy[dir];

            if(0<=nX && nX < N && 0<=nY && nY < N){
                if(board[nX][nY] == WHITE){
                    ArrayList<Pawn> temp = new ArrayList<>(pawnGraph[x][y].subList(targetIdx,pawnGraph[x][y].size()));
                    pawnGraph[x][y]= new ArrayList<>(pawnGraph[x][y].subList(0,targetIdx));
                    for (Pawn p : temp) {
                        p.x = nX;
                        p.y = nY;
